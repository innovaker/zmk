...
struct active_sticky_key {
    ...
    struct hid_usage modified_key_usage;
};
...
static struct active_sticky_key *store_sticky_key(uint32_t position, uint32_t param1,
    ...
    for (int i = 0; i < ZMK_BHV_STICKY_KEY_MAX_HELD; i++) {
        ...
        sticky_key->modified_key_usage.page = 0;
        sticky_key->modified_key_usage.id = 0;
        return sticky_key;
    }
    return NULL;
}
...
static int on_sticky_key_binding_released(struct zmk_behavior_binding *binding,
    ...
    if (sticky_key->modified_key_usage.page != 0 && sticky_key->modified_key_usage.id != 0) {
...
static int sticky_key_keycode_state_changed_listener(const struct zmk_event_header *eh) {
    ...
    for (int i = 0; i < ZMK_BHV_STICKY_KEY_MAX_HELD; i++) {
        ...
        if (strcmp(sticky_key->config->behavior.behavior_dev, "KEY_PRESS") == 0) {
            struct zmk_code code = zmk_code_unpack(sticky_key->param1);
            if (code.usage.page == ev->usage.page && code.usage.id == ev->usage.id &&
                code.modifiers == ev->implicit_modifiers) {
        ...
        if (ev->state) { // key down
            if (sticky_key->modified_key_usage.page != 0 ||
                sticky_key->modified_key_usage.id != 0) {
            ...
            sticky_key->modified_key_usage.page = ev->usage.page;
            sticky_key->modified_key_usage.id = ev->usage.id;
